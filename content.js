...
function main() {
    ...
    chrome.storage.local.get(null, (all) => console.log("All storage:", all));
    ...
    window.addEventListener('console-log-intercepted', (e) => {
        ...
        if (!/SBCL:/.test(message[0])) { // Ignores logs from the post response interceptor
            ...
            if (Array.isArray(message) && message[0] == ("[ACT] START")) { //* Begin question
                ...
                chrome.storage.local.get("iterator", function(iter) {
                    const iter_value = iter.iterator?.value ?? 11; // Can never be 11
                    ...
                    setTimeout(() => {
                        ...
                        (async () => {
                            if (last_homework_type == null || last_homework_type == undefined) {
                                ...
                                homework_type = type?.value ?? 7; // 7 = error getting
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "SETUP_COMPLETE") {
        main(); // Allows background.js time to 
    }
});

// In case message send before content.js loads after setup
chrome.runtime.sendMessage({ type: "GET_SETUP_STATUS" }, (response) => {
    if (response && response.setupComplete && !main_running) {
        main();
    }
});